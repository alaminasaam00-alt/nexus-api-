...
app = FastAPI(title="NEXUS LaunchDesk", version="0.4.2")
...
def page() -> str:
    cards = "".join("<article class='card'><h3>" + o["name"] + "</h3><p>" + o["description"] + "</p><strong>$" + str(o["price_usd"]) + "</strong><small>" + o["delivery"] + "</small><button onclick=\"choose('" + o["id"] + "')\">اطلب الآن</button></article>" for o in OFFERS)
    options = "".join("<option value='" + o["id"] + "'>" + o["name"] + "</option>" for o in OFFERS)
    html = """<!doctype html>
<html lang='ar' dir='rtl'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>NEXUS LaunchDesk</title>
<style>body{font-family:system-ui;background:#0b1020;color:#f4f7ff;margin:0}main{max-width:1000px;margin:auto;padding:28px}.hero{padding:32px 0}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:16px}.card,form{background:#151d33;border:1px solid #2d3a5e;border-radius:18px;padding:20px}.card strong{display:block;font-size:28px;margin:16px 0}.card small{display:block;color:#aebbd8;margin-bottom:16px}button{background:#6d8cff;color:white;border:0;border-radius:10px;padding:12px 16px;font-weight:700;cursor:pointer}input,textarea,select{width:100%;box-sizing:border-box;margin:8px 0 14px;padding:12px;border-radius:10px;border:1px solid #3a486d;background:#0f1629;color:white}#result{margin-top:15px;color:#9ef0b5}</style></head>
<body><main><section class='hero'><p>خدمات رقمية سريعة للأفراد وأصحاب المشاريع</p><h1>NEXUS LaunchDesk</h1><p>اطلب خدمة واضحة واستلمها عن بُعد.</p></section><section class='grid'>__CARDS__</section>
<form id='order' onsubmit='submitOrder(event)'><h2>إرسال طلب</h2><label>الخدمة</label><select id='offer' required>__OPTIONS__</select><label>الاسم</label><input id='name' required minlength='2'><label>واتساب أو بريد إلكتروني</label><input id='contact' required minlength='4'><label>التفاصيل</label><textarea id='notes' rows='4'></textarea><button type='submit'>إرسال الطلب</button><div id='result'></div></form></main>
<script>
function choose(id){document.getElementById('offer').value=id;document.getElementById('order').scrollIntoView({behavior:'smooth'});}
async function submitOrder(e){e.preventDefault();const result=document.getElementById('result');result.textContent='جارٍ إرسال الطلب...';try{const r=await fetch('/api/v1/orders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({offer_id:document.getElementById('offer').value,customer_name:document.getElementById('name').value,contact:document.getElementById('contact').value,notes:document.getElementById('notes').value})});const data=await r.json();result.textContent=r.ok?'تم استلام طلبك رقم '+data.order_id+'. سيتم التواصل معك لتأكيد الدفع.':(data.detail||'تعذر إرسال الطلب');}catch(err){result.textContent='تعذر الاتصال بالخادم. حاول لاحقاً.';}}
</script></body></html>"""
    return html.replace("__CARDS__", cards).replace("__OPTIONS__", options)
...
def create_order(payload: OrderRequest) -> dict[str, Any]:
    ...
    order_id = "ORD-" + uuid4().hex[:10].upper()
    with db() as conn:
...
def status() -> dict[str, Any]:
    return {"service": "nexus-launchdesk", "version": app.version, "storefront": True, "orders": True, "persistent_storage": DB_PATH.parent.as_posix() == "/data", "admin_dashboard": True, "payment_provider": "not_configured", "settlement": "owner_controlled"}

@app.get("/api/v1/safety")
def safety() -> dict[str, Any]:
    mode = os.getenv("TRADING_MODE", "disabled").strip().lower()
    return {"trading_mode": mode, "binance_environment": os.getenv("BINANCE_ENV", "testnet"), "live_orders_enabled": False, "withdrawals_enabled": False, "manual_approval_required": True, "kill_switch": True, "status": "safe" if mode == "disabled" else "restricted"}
