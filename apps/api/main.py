...
app = FastAPI(
    title="NEXUS LaunchDesk",
    version="0.3.0",
    description="A lightweight Arabic-first service storefront and lead intake API. Payments require a configured provider and owner-controlled settlement account.",
)
...
OFFERS = [
    {
        ...
        "description": "سيرة ذاتية عربية أو إنجليزية متوافقة مع أنظمة ATS.",
        "price_usd": 12,
        "delivery": "24-48 ساعة",
    },
    {
        "id": "business-kit",
        "name": "حزمة إطلاق مشروع صغير",
        "description": "اسم تجاري، وصف خدمات، عرض أسعار، ونصوص تسويقية جاهزة.",
        "price_usd": 25,
        "delivery": "48-72 ساعة",
    },
    {
        "id": "automation-audit",
        "name": "مراجعة أتمتة الأعمال",
        "description": "تحديد 5 مهام قابلة للأتمتة مع خطة تنفيذ عملية.",
        "price_usd": 35,
        "delivery": "48-72 ساعة",
    },
]
...
def storefront() -> str:
    cards = "".join(
        f"<article class='card'><h3>{o['name']}</h3><p>{o['description']}</p><strong>${o['price_usd']}</strong><small>{o['delivery']}</small><button onclick=\"choose('{o['id']}','{o['name']}')\">اطلب الآن</button></article>"
        for o in OFFERS
    )
    return f"""<!doctype html><html lang='ar' dir='rtl'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>NEXUS LaunchDesk</title><style>body{{font-family:system-ui;background:#0b1020;color:#f4f7ff;margin:0}}main{{max-width:1000px;margin:auto;padding:28px}}.hero{{padding:32px 0}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:16px}}.card,form{{background:#151d33;border:1px solid #2d3a5e;border-radius:18px;padding:20px}}.card strong{{display:block;font-size:28px;margin:16px 0}.card small{{display:block;color:#aebbd8;margin-bottom:16px}}button{{background:#6d8cff;color:white;border:0;border-radius:10px;padding:12px 16px;font-weight:700;cursor:pointer}}input,textarea,select{{width:100%;box-sizing:border-box;margin:8px 0 14px;padding:12px;border-radius:10px;border:1px solid #3a486d;background:#0f1629;color:white}}#result{{margin-top:15px;color:#9ef0b5}}</style></head><body><main><section class='hero'><p>خدمات رقمية سريعة للأفراد وأصحاب المشاريع</p><h1>NEXUS LaunchDesk</h1><p>اطلب خدمة واضحة، استلمها عن بُعد، وابدأ دون تعقيد.</p></section><section class='grid'>{cards}</section><form id='order' onsubmit='submitOrder(event)'><h2>إرسال طلب</h2><label>الخدمة</label><select id='offer' required></select><label>الاسم</label><input id='name' required minlength='2'><label>واتساب أو بريد إلكتروني</label><input id='contact' required minlength='4'><label>التفاصيل</label><textarea id='notes' rows='4'></textarea><button type='submit'>إرسال الطلب</button><div id='result'></div></form></main><script>const offers={[("'"+o["id"]+"':'"+o["name"].replace("'"," ")+"'") for o in OFFERS]};const select=document.getElementById('offer');Object.entries(offers).forEach(([id,name])=>select.add(new Option(name,id)));function choose(id,name){{select.value=id;document.getElementById('order').scrollIntoView({{behavior:'smooth'}})}}async function submitOrder(e){{e.preventDefault();const result=document.getElementById('result');result.textContent='جارٍ إرسال الطلب...';const r=await fetch('/api/v1/orders',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{offer_id:select.value,customer_name:document.getElementById('name').value,contact:document.getElementById('contact').value,notes:document.getElementById('notes').value}})}});const data=await r.json();result.textContent=r.ok?'تم استلام طلبك. سيتم التواصل معك لتأكيد السعر وطريقة الدفع.':(data.detail||'تعذر إرسال الطلب');}}</script></body></html>"""
...
def create_order(payload: OrderRequest) -> dict[str, Any]:
    ...
    ORDERS.append(order)
    return {"accepted": True, "order_id": order["id"], "status": order["status"], "next_step": "owner_contact_required"}
